...
class GraphQLMangaRepositoryImpl @Inject constructor(
    ...
    private fun fetchMangaById(id: Long): Single<MangaDetails> =
        fetchMangaByIdRaw(id).map { it.toDomainDetails(id) }

    private fun fetchMangaByIdRaw(id: Long): Single<MangaByIdQuery.Manga> =
        apolloClient.rxQuery(MangaByIdQuery(ids = id.toString()))
            .map { response ->
                response.mangas.firstOrNull()
                    ?: throw NoSuchElementException("Manga $id not found")
            }
    ...
    private fun MangaListQuery.AiredOn?.toDomainDate(): DateTime? {
        ...
        return date?.tryParseDateTime() ?: run {
    ...
    private fun MangaByIdQuery.Manga.toDomainDetails(mangaId: Long): MangaDetails = MangaDetails(
        ...
        genres = genres?.toDomainGenres() ?: emptyList(),
    ...
    /**
     * Full poster from MangaByIdQuery (has originalUrl, previewUrl, mainUrl).
     */
    private fun MangaByIdQuery.Poster?.toDomainImage(): Image = when (this) {
        null -> Image(null, null, null, null)
        else -> Image(
    ...
    private fun MangaByIdQuery.UserRate?.toDomainUserRate(targetId: Long?): UserRate? {
        ...
        return UserRate(
            ...
            text = text,
            textHtml = null,
            dateCreated = createdAt.tryParseDateTime(),
            dateUpdated = updatedAt.tryParseDateTime()
        )
    }
    ...
    private fun MangaByIdQuery.AiredOn?.toDomainDate(): DateTime? {
        if (this == null) return null
        return date?.tryParseDateTime() ?: run {
            val y = year ?: return@run null
            val m = month ?: 1
    ...
    private fun MangaByIdQuery.ReleasedOn?.toDomainDate(): DateTime? {
        if (this == null) return null
        return date?.tryParseDateTime() ?: run {
            val y = year ?: return@run null
            val m = month ?: 1
    ...
    private fun List<MangaByIdQuery.Genre>.toDomainGenres(): List<Genre> =
        mapNotNull { gqlGenre ->
            val normalizedName = gqlGenre.name
                .replace(Regex("[-\\s]"), "_")
                .lowercase()
            Genre.values().firstOrNull { it.equalsName(normalizedName) }
        }
    ...
    private fun Any?.tryParseDateTime(): DateTime? {
        return when (this) {
            is String -> try {
