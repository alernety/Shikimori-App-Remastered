...
import com.gnoemes.shikimori.databinding.ItemRateSortBinding
...
class RateSortAdapterDelegate(
    ...
    override fun onCreateViewHolder(parent: ViewGroup): ViewHolder =
            ViewHolder(ItemRateSortBinding.inflate(android.view.LayoutInflater.from(parent.context), parent, false))

    override fun onBindViewHolder(item: RateSortViewModel, holder: ViewHolder, payloads: MutableList<Any>) {
        holder.bind(item)
    }

    inner class ViewHolder(private val binding: ItemRateSortBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        init {
            binding.sortView.onClick { callback.invoke(SortAction.ChangeSort(item.sorts.map { Triple(it.first, it.second, it.first == item.currentSort) })) }
            binding.orderView.onClick {
                isDescending = !isDescending
                callback.invoke(SortAction.ChangeOrder(isDescending))
                binding.orderView.setImageResource(iconRes)
            }
        }
        ...
        private fun getLocalizedStatus(status: RateStatus, anime: Boolean): String =
                if (anime)
                    RateStatus.values()
                            .zip(binding.root.resources.getStringArray(R.array.anime_rate_stasuses))[status.ordinal]
                            .second
                else RateStatus.values()
                        .zip(binding.root.resources.getStringArray(R.array.manga_rate_stasuses))[status.ordinal]
                        .second
